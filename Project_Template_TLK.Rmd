...
# Summary infos of the monthly mean precip. of beaufort 
summary(Beaufort_Clean)
...
#summary of Early precip 
summary(Beaufort_early)
...
ggplot(Beaufort_earlyNo0Precip, aes(x=Date , y=sigPrecip))+
  geom_point()
...
Beaufort_Late<- Beaufort_RAW%>%
  ...
  mutate(sigPrecip= ifelse(PrecipInches>3.66,PrecipInches,0),
         NumSigPrecip= ifelse(PrecipInches>3.66, 1,NA))%>%
  select(Date, year, month, 
         day_of_month, PrecipInches, sigPrecip, NumSigPrecip)%>%
  drop_na()

#Summary of Late precip 

summary(Beaufort_Late)
...
Beaufort_TS<- ts(Beaufort_RAW$Area.Weighted.Mean.Precipitation..mm.per.day., start = c())
```

```{r t-test for decades}
t.test(Beaufort_early$PrecipInches, Beaufort_Late$PrecipInches)
#not significant
#Precipitation not different over decades

t.test(Beaufort_early$sigPrecip, Beaufort_Late$sigPrecip)
#significant!
#More 1 year events in late decades!
```
