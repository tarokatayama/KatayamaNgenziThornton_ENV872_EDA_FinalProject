...
**Initial Wrangling**
```{r setup2}
knitr::opts_chunk$set(tidy.opts=list(width=50), tidy=TRUE)
...
```{r early plot 1, fig.align='center', fig.width= 6, fig.cap="Sig events (early decade)"}
...
Beaufort_early$Date<- as.Date(Beaufort_early$Date, "%Y-%m-%d")
...
```
\newpage
```{r late plot 1, fig.align='center', fig.cap= "Sig events (late decade)", fig.width= 6, echo=TRUE}
...
```{r late plot 2, fig.align='center', fig.width=6, fig.cap="Overall Precipitation (late decade)"}
#Create a figure showing the overall precipitation for the late decade
Beaufort_Late$Date<- as.Date(Beaufort_Late$Date, "%Y-%m-%d")

Plot_late_overall <- ggplot(Beaufort_Late, 
                            aes(x=Date , y=PrecipInches))+
  geom_point()+
  scale_x_date(date_labels = ("%m-%Y"))+
  labs(y="Precipitation (in)", x= "Date")
